...
const wchar_t kClientModule[] = L"client.dll";
const int kOffset1 = 0xDE997C;
const int kOffset2 = 0x312;
const int kGap = 4;

ProcessReader::ProcessReader() {
  HWND hWnd = FindWindowA(NULL, "csgo.exe");
  DWORD dwPid = -1;
  ...
  player_base_addr_ =
      MemoryUtil::RPM<int>((HANDLE)game_handle_, (DWORD)clientBase + kOffset1);
}

void ProcessReader::UpdateUserPosition() {
  float player_position_x =
      MemoryUtil::RPM<float>(game_handle_, player_base_addr_ + kOffset2);
  float player_position_y =
      MemoryUtil::RPM<float>(game_handle_, player_base_addr_ + kOffset2 + kGap);
